...
void DebugPointRenderer::Shutdown()
{
    // Destroy Pipeline
    ...
    vkDestroyFramebuffer(Graphics::GetVkDevice(), outputFrameBuffer, nullptr);
...
void DebugPointRenderer::CreateImages()
{
    outputImage.CreateImage(VK_FORMAT_R8G8B8A8_UNORM, 1024, 1024,
        VK_IMAGE_USAGE_SAMPLED_BIT | VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT | VK_IMAGE_USAGE_TRANSFER_SRC_BIT);
    outputImage.CreateImageView();

    depthImage.CreateImage(VulkanSetup::GetDepthBufferFormat(Graphics::GetVkPhysicalDevice()), 1024, 1024,
        VK_IMAGE_USAGE_DEPTH_STENCIL_ATTACHMENT_BIT);
    depthImage.CreateImageView(true);

    outputImageImGuiTex = reinterpret_cast<ImTextureID>(
        ImGui_ImplVulkan_AddTexture(Graphics::GetBasicLinearSampler(), outputImage.GetImageView(), VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL)
    );
}

void DebugPointRenderer::CreateRenderPass()
{
    VkAttachmentDescription colorAttachment{};
    colorAttachment.format = outputImage.GetImageFormat();
...
void DebugPointRenderer::CreateFrameBuffer()
{
    VkImageView imageViewList[]{ outputImage.GetImageView(), depthImage.GetImageView() };

    VkFramebufferCreateInfo frameBufferCreateInfo{};
    frameBufferCreateInfo.sType = VK_STRUCTURE_TYPE_FRAMEBUFFER_CREATE_INFO;
    frameBufferCreateInfo.width = outputImage.GetImageExtent().width;
    frameBufferCreateInfo.height = outputImage.GetImageExtent().height;
    frameBufferCreateInfo.attachmentCount = 2;
    frameBufferCreateInfo.pAttachments = imageViewList;
    frameBufferCreateInfo.renderPass = vkRenderPass;
    frameBufferCreateInfo.layers = 1;

    if (vkCreateFramebuffer(Graphics::GetVkDevice(), &frameBufferCreateInfo, nullptr, &outputFrameBuffer) != VK_SUCCESS) {
        throw - 1;
    }
}
...
void DebugPointRenderer::CreatePipeline()
{
    // Get shader code
    ...
    VulkanUtility::DebugNameObject("DebugPointRenderer_UGP_PipelineLayout", reinterpret_cast<uint64_t>(vkMainPipelineLayout), VK_OBJECT_TYPE_PIPELINE_LAYOUT);
    ...
    VulkanUtility::DebugNameObject("DebugPointRenderer_UGP_Pipeline", reinterpret_cast<uint64_t>(vkMainPipeline), VK_OBJECT_TYPE_PIPELINE);
...
void DebugPointRenderer::BeginRender(HMM_Vec4 clearColor, VkCommandBuffer commandBuffer)
{
    ...
    renderPassInfo.renderArea.extent = outputImage.GetImageExtent();
