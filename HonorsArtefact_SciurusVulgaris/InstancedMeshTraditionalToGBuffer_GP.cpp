...
void InstancedMeshTraditionalToGBuffer_GP::CreatePipeline(const VkRenderPass& vkRenderPass)
{
    // Get shader code

    auto vertShaderCode = VulkanUtility::ReadFile("./COMPILEDSHADER_InstanceMeshVertex.spv");
    auto fragShaderCode = VulkanUtility::ReadFile("./COMPILEDSHADER_DeferredMeshNoVelocity.spv");
    ...
    VulkanUtility::DebugNameObject("InstancedMeshTraditionalToGBuffer_GP_PipelineLayout", reinterpret_cast<uint64_t>(vkPipelineLayout), VK_OBJECT_TYPE_PIPELINE_LAYOUT);
    ...
    VulkanUtility::DebugNameObject("InstancedMeshTraditionalToGBuffer_GP_Pipeline", reinterpret_cast<uint64_t>(vkPipeline), VK_OBJECT_TYPE_PIPELINE);
...
void InstancedMeshTraditionalToGBuffer_GP::Shutdown() {
