...
void MeshRenderer::CreatePipeline()
{
    ...
    auto vertShaderCode = VulkanUtility::ReadFile("./COMPILEDSHADER_MeshVertexPhotogrammetric.spv");
    auto fragShaderCode = VulkanUtility::ReadFile("./COMPILEDSHADER_MeshFragmentPhotogrammetric.spv");
...
void MeshRenderer::ExtractPoints(TriListMesh** meshes, uint64_t meshCount, HMM_Vec3 viewingFrom, HMM_Vec3 upDirection)
{
    ...
    for (int meshIndex = 0; meshIndex < meshCount; meshIndex++) {
        WCP_Matrices dataForUBO{
            HMM_Rotate_LH(3.141 / 2.0, HMM_V3(1, 0, 0)) * HMM_Scale(HMM_V3(0.03, 0.03, 0.03)), HMM_LookAt_LH(viewingFrom, viewingFrom - (HMM_V3(0, 0, 0) - viewingFrom), -upDirection), HMM_Orthographic_RH_ZO(-1.5, 1.5, -2.2, 0.5, 0.001, 10)
        };
        meshes[meshIndex]->GetDescriptorSet()->UpdateUniformBufferData(0, &dataForUBO);
    }
    BeginRender(HMM_V4(0, 0, 0, 0), thisExtractionCommandBuffer);
    for (int meshIndex = 0; meshIndex < meshCount; meshIndex++) Render(meshes[meshIndex], thisExtractionCommandBuffer);
