...
void TAA_GP::CreateDescriptorLayout()
{
    ...
    uboLayoutBindings[1].binding = 1; // History Input
...
void TAA_GP::CreatePipeline(const VkRenderPass& vkRenderPass)
{
    // Get shader code

    auto vertShaderCode = VulkanUtility::ReadFile("./COMPILEDSHADER_DeferedQuad.spv");
    auto fragShaderCode = VulkanUtility::ReadFile("./COMPILEDSHADER_TAAFrag.spv");
    ...
    VulkanUtility::DebugNameObject("TAA_GP_PipelineLayout", reinterpret_cast<uint64_t>(vkPipelineLayout), VK_OBJECT_TYPE_PIPELINE_LAYOUT);
    ...
    VulkanUtility::DebugNameObject("TAA_GP_Pipeline", reinterpret_cast<uint64_t>(vkPipeline), VK_OBJECT_TYPE_PIPELINE);
